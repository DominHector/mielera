...
{{*                        <a class="btn-floating halfway-fab waves-effect waves-light red"><i class="material-icons">add</i></a>*}}
                    ...
                    <div class="card-body">
                        ...
                        {{else}}
                            {{if !empty($elm['ml_purchase_quantity'])}}
                                <span class="card-info yellow-text text-darken-2 left"><b>{{$elm['ml_purchase_quantity']}}</b>
                                {{if !empty($elm['ml_purchase_sale'])}}
                                    {{if $elm['ml_purchase_sale'] == 'selling'}}
                                        <b>procesando</b>
                                    {{else}}
                                        <b>comprado</b>
                                    {{/if}}
                                {{/if}}
                                {{if $elm['ml_purchase_quantity'] > 1}}s{{/if}} el <b>{{$elm['ml_purchase_date']}}</b></span>
                            {{/if}}

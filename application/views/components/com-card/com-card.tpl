            ...
            <div class="com-card col-xs-12 col-lg-{{12/$col}}" data-product-price="{{$elm['ml_product_price']}}" data-product-id="{{$elm['ml_product_id']}}">
                <div class="card {{$type}}">
                    ...
                    <div class="card-body">
                        ...
                        {{else}}
                            {{if !empty($elm['ml_purchase_quantity'])}}<span class="yellow-text text-darken-2 left"><b>{{$elm['ml_purchase_quantity']}}</b> comprado{{if $elm['ml_purchase_quantity'] > 1}}s{{/if}} el <b>{{$elm['ml_purchase_date']}}</b></span>{{/if}}
